...
import { getSistemItem, sistemModuleItems } from "@/lib/sistem-module";
import { getSistemGroup, getSistemSection } from "@/lib/sistem-submenus";
import { notFound } from "next/navigation";

interface SistemSectionPageProps {
  params: Promise<{ section: string }>;
}

export default async function SistemSectionPage({ params }: SistemSectionPageProps) {
  const { section: sectionId } = await params;
  const section = getSistemSection(sectionId);
  const directItem = getSistemItem(sectionId);
  const parentGroup = directItem ? getSistemGroup(directItem.group) : undefined;
  ...
  if (section) {
    return (
      <div className="space-y-6">
        ...
        <Link href={parentGroup?.href ?? "/sistem"} className="btn-ghost inline-flex">
...
export function generateStaticParams() {
  return sistemModuleItems.map((item) => ({ section: item.id }));
}
