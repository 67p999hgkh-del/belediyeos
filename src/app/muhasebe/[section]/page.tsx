...
import { getMuhasebeItem, muhasebeModuleItems } from "@/lib/muhasebe-module";
import { getMuhasebeGroup, getMuhasebeSection } from "@/lib/muhasebe-submenus";
import { notFound } from "next/navigation";

interface MuhasebeSectionPageProps {
  params: Promise<{ section: string }>;
}

export default async function MuhasebeSectionPage({ params }: MuhasebeSectionPageProps) {
  const { section: sectionId } = await params;
  const section = getMuhasebeSection(sectionId);
  const directItem = getMuhasebeItem(sectionId);
  const parentGroup = directItem ? getMuhasebeGroup(directItem.group) : undefined;
  ...
  if (section) {
    return (
      <div className="space-y-6">
        ...
        <Link href={parentGroup?.href ?? "/muhasebe"} className="btn-ghost inline-flex">
...
export function generateStaticParams() {
  return muhasebeModuleItems.map((item) => ({ section: item.id }));
}
