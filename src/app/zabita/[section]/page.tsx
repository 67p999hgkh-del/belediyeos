...
import { getZabitaItem, zabitaModuleItems } from "@/lib/zabita-module";
import { getZabitaGroup, getZabitaSection } from "@/lib/zabita-submenus";
import { notFound } from "next/navigation";

interface ZabitaSectionPageProps {
  params: Promise<{ section: string }>;
}

export default async function ZabitaSectionPage({ params }: ZabitaSectionPageProps) {
  const { section: sectionId } = await params;
  const section = getZabitaSection(sectionId);
  const directItem = getZabitaItem(sectionId);
  const parentGroup = directItem ? getZabitaGroup(directItem.group) : undefined;
  ...
  if (section) {
    return (
      <div className="space-y-6">
        ...
        <Link href={parentGroup?.href ?? "/zabita"} className="btn-ghost inline-flex">
...
export function generateStaticParams() {
  return zabitaModuleItems.map((item) => ({ section: item.id }));
}
